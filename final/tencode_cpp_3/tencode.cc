...
string get_word(ifstream & in) { // std::ifstream is non-copyable => need to pass by reference!
    string s;
    char c;
    while ((in.get(c))) {
        if (!isalpha(c)) {
...
int main (int argc, char * argv[]) {
    ...
    ifstream in1("tempfile");
    build_vocab(in1); // pass 1
