#include <iostream>
#include <fstream>
#include <sstream>
#include <string>
#include <vector>


unsigned num_colons(std::string & s) {
    unsigned N = 0;
    for (int i = 0; i < s.length(); i++) {
        if (s[i] == ':') N++;
    }
    return N;
}

void get_rules(char * filename, std::vector<std::string> & P, std::vector<std::string> & T) {
    std::ifstream input(filename);
    if (!input) {
        std::cout << "invalid rules file " << filename << std::endl;
        exit(EXIT_FAILURE);
    }

    std::string line;
    std::string p, t;

    while (std::getline(input, line)) {
        if (num_colons(line) == 0) continue;

        if (line[0] == ':') {
            if (T.size() == 0) {
                std::cout << "invalid rules file " << filename << std::endl;
                exit(EXIT_FAILURE);
            }
            line[0] = '\n';
            T[T.size() - 1] += line;
        } else {
            p = line.substr(0, line.find(":"));
            t = line.substr(line.find(":") + 1);
            P.push_back(p);
            T.push_back(t);
        }
    }
}


std::string get_match(std::string & line, unsigned & i, std::vector<std::string> & P, std::vector<std::string> & T) {
    for (int j = 0; j < P.size(); j++) {
        if (line.substr(i, P[j].length()) == P[j]) {
            i += P[j].length() - 1; // -1 because it will get incremented again
            return T[j];
        }
    }
    return std::string(1, line[i]);
}

void expand(std::istream & input, std::ostream & output, std::vector<std::string> & P, std::vector<std::string> & T) {
    std::string line;
    while (std::getline(input, line)) {
        for (unsigned i = 0; i < line.length(); i++) {
            output << get_match(line, i, P, T);
        }
        output << std::endl;
    }
}


int main(int argc, char * argv[]) {

    std::vector<std::string> P;
    std::vector<std::string> T;

    if (argc == 1) {
        get_rules("RULES", P, T);
    }
    for (int i = 1; i < argc; i++) {
        get_rules(argv[i], P, T);
    }

    expand(std::cin, std::cout, P, T);
}
